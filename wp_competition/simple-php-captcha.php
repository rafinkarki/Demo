...
//
//  Copyright 2015 by Andmine for A Beautiful Site, LLC
//
...
if( isset($_GET['_CAPTCHA']) ) {
    ...
    $textcolor1 = imagecolorallocate($captcha, 39, 40, 34);
    $textcolor2 = imagecolorallocate($captcha,221, 78, 65);
    ...
    // set background and allocate drawing colours
      $background = imagecolorallocate($captcha, 0x66, 0xCC, 0xFF);
      imagefill($captcha, 0, 0, $background);
      $linecolor = imagecolorallocate($captcha, 0x33, 0x99, 0xCC);
      
   // draw random lines on canvas
    for($i=0; $i < 8; $i++) {
        imagesetthickness($captcha, rand(1,3));
        imageline($captcha, rand(0,160), 0, rand(0,160), 73, $linecolor);
    }
    $pixel_color = imagecolorallocate($captcha, 0,0,255);
    for($i=0;$i<1000;$i++) {
        imagesetpixel($captcha,rand()%200,rand()%100,$pixel_color);
    }  
    // Draw text
